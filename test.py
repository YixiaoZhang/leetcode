...
def filter(currentWord, wordList, connectDic):
    output = []
    matchedList = connectDic[currentWord]
    for word in wordList:
        if word in matchedList:
            output.append(word)
    ...
    for j in range(len(wordList)):
        if(i!=j):
            ...
            for k in range(len(currentWord)):
                if(wordList[j][k]!=currentWord[k]):
                    diff += 1
                if(diff>1):
                    break
            if(diff==1):
                l.append(wordList[j])
    connectDic[currentWord] = l
...
while(len(queue)>0):
    ...
    for word in wordList:
        if word not in head:
            left.append(word)
    available = filter(head[-1],left, connectDic)
